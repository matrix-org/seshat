...
describe('Database', function() {
  ...
  it('should throw an error when adding events with missing fields.', function() {
    ...
    expect(() => db.add_event(matrixEvent)).to.throw("Event doesn't contain any content");

    delete matrixEvent.room_id;
    expect(() => db.add_event(matrixEvent)).to.throw("Event doesn't contain a valid room id");

    delete matrixEvent.origin_server_ts;
    expect(() => db.add_event(matrixEvent)).to.throw("Event doesn't contain a valid timestamp");

    delete matrixEvent.event_id;
    expect(() => db.add_event(matrixEvent)).to.throw("Event doesn't contain a valid event id");

    delete matrixEvent.sender;
    expect(() => db.add_event(matrixEvent)).to.throw("Event doesn't contain a valid sender");
  });

  it('should throw an error when adding events with fields that don\'t typecheck.', function() {
    expect(() => db.add_event(badEvent)).to.throw("Event doesn't contain a valid timestamp");
  });
